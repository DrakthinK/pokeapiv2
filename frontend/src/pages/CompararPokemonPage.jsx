      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        <div className="md:flex md:place-items-center mb-6">
          <div className="mb-4 w-11/12">
            <label className="block text-gray-500 font-bold md:text-left mb-1 md:mb-0 pr-4" htmlFor="Primer Pokemon">
              Primer Pokemon
            </label>
            <input
        ...
        <div className="flex md:place-items-center justify-between">
          <button type="submit" className="shadow bg-purple-500 hover:bg-purple-400 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded">Comparar</button>
        {ganador && <p>El ganador es: {ganador.name}</p>}
        </div>
        <Card name={ganador.name} image={ganador.front_default} id={ganador.id} key={ganador.id} />
      </form>
      </div>
    </>
  );
